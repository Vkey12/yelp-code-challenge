...
class Restaurant:
    ...
    @classmethod
    def all_restaurants(cls):
        return [r for r in Restaurant.all if r._reviews]

    @classmethod
    def top_two_restaurants(cls):
        if not cls.all_restaurants():
            return None
        else:
            # sort the list of restaurants in descending order by their average star rating
